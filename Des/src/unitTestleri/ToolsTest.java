...
public class ToolsTest {
    ...
    public void toStr_Test_keldsdime() throws Exception {
        Assert.assertEquals("1011000010110010", dES.toBinary("ŞĞ"));
        Assert.assertEquals("0110010001100101011011100110010101101101011001010100010000111111001011100010101000111101", dES.toBinary("denemeD?.*="));
        Assert.assertEquals("11100111011001000110010101101110011001010110110101100101010001000011111100101110001010100011110110110100", dES.toBinary("çdenemeD?.*=İ"));
    }
    @Test
    public void toBinary_Test() throws Exception {
        //Tek harf
        Assert.assertEquals("00101010", dES.toBinary("*"));
        Assert.assertEquals("01100001", dES.toBinary("a"));
        Assert.assertEquals("11100111", dES.toBinary("ç"));
        Assert.assertEquals("11000111", dES.toBinary("Ç"));
        Assert.assertEquals("01011110", dES.toBinary("Ş"));
        Assert.assertEquals(
                "11110110" + //hatasız ö
                        "11010110" + //hatasız Ö
                        "11111100" + //hatasız ü
