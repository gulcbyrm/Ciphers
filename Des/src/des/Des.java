...
import sabitler.EncriptDecriptSabitleri;
import exceptions.KeyLengthUygunDeil;
import sabitler.Mode;
...
public class Des extends Abstract {
    ...
    public static List<String> islemDetayList=new ArrayList<>();

    //şifreleme işini gerçekleştiren metod
    public String encript(final String plainText, final String key) throws UnsupportedEncodingException, KeyLengthUygunDeil {
        return sifrele(plainText, key, Mode.Encript);
    }


    //şifre çözen metod
    public String decript(final String encriptedText, final String key) throws UnsupportedEncodingException, KeyLengthUygunDeil {
    ...
    private String sifrele(final String plainText, String key, Mode mod ) throws UnsupportedEncodingException, KeyLengthUygunDeil {
        ...
        final Araclar araclar = new Araclar();                                              // des yardımcı araçlarının olduğu class tobinary vs..
        ...
        for (String blockText : plaintTextList) {                                           //her 8 li block alınıp şifrelenmeye gönderiliyor
            final String biteCevrilmisHali = araclar.toBinary(blockText);  //64BİT          //bloklar bite çevriliyor toplam:64 bit
            islemDetayList.add("--------------------------------------------------------------------------------------------------------------------");
            islemDetayList.add("şifrelenecek blok [String]: "+blockText);
            islemDetayList.add("şifrelenecek blok [Binary]: "+biteCevrilmisHali);
           ...
           final String encriptedBlock = mod == Mode.Encript ? araclar.toHex64BitBinary(R16SonKaristirma) : araclar.toStr(R16SonKaristirma);//*****
            ...
            islemDetayList.add("L16+R16 AfterPerm\t: "+ R_arti_L);
            //islemDetayList.add("R+L[16] P tablosuyla Permütasyon: "+ R16SonKaristirma);
            islemDetayList.add("Şifrelenmiş Block\t: "+ encriptedBlock);
        }

        islemDetayList.add("Şifrelenmiş Metin\t: "+ encriptedString.toString());
        islemDetayList.add("--------------------------------------------------------------------------------------------------------------------");
    ...
    /**
     * DES şifrelerken 56 bite indirgenmiş olan binary veriyi sol ve sağ diye ikiye böler daha sonra
     ...
     * @param key kullanıcının belirlediği key
     ...
     */
    private String onAltiRounduIsle(String initPermSonrasiBit,String key, Mode mod) throws KeyLengthUygunDeil {
    final EncriptDecriptSabitleri sbt = new EncriptDecriptSabitleri();                  //tabloların bulunduğu class

//64
    final String[] subKeys = subKeyUret(key);                                           //subKeyUret fonksiyonu 16 adet 48 bite indirgenmiş key üretir
    ...
    for (int i = 1; i <= 16; i++) {
        ...
        islemDetayList.add("Round "+i+"\t\tLeft: "+ L[i]+"\t\tRight: "+ R[i] );
    }
    return R[16]+L[16];
}
